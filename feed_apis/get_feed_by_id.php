...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_array($decoded)) {
        ...
        if (isset($decoded['myUserId'])) {
            ...
            $feed = null;
            $query = "SELECT * FROM `post`, users where users.primary_id = post.user_id and post.post_id = $postId ORDER by post_id desc";
            ...
            $response = array('response' => 'success', 'responseCode' => 200, 'data' =>$feed
            );
            echo json_encode($response);
